...
// Education-only types
export interface EducationAgentNodeData {
...
export const EducationAgentNode = memo(({ data, selected, id, dragging }: ExtendedNodeProps) => {
  ...
  return (
    <div className="group">
      <Handle
        ...
        id="agent-input"
        className="!w-3 !h-3 !bg-gradient-to-r !from-green-500 !to-emerald-500 !border-2 !border-background"
        style={{ top: '50%' }}
      />
      ...
      <Card className={`relative w-80 p-5 border-muted/50 shadow-xl bg-gradient-to-b from-background to-background/90 backdrop-blur-sm transition-all duration-300 hover:shadow-2xl ${
        ...
        <div className="flex items-center gap-2">
          ...
          {data.status === "ready" && data.draft && data.outputId ? (
            <>
              <Button 
                size="sm" 
                variant="outline" 
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent>
                  {config.downloadFormats.map((format) => (
                    <DropdownMenuItem 
                      key={format}
                      onClick={() => data.onDownload?.(format)}
                    >
                      {format.toUpperCase()}
                    </DropdownMenuItem>
                  ))}
...
EducationAgentNode.displayName = "EducationAgentNode";
